...
def _safe_redis_set(cache_conn, key, value, ex=None):
    try:
        # NOTE: استفاده از default=str برای اطمینان از serializabl بودن datetime و دیگر اشیا
...
class GlobalSearchView(APIView):
    ...
    def get(self, request):
        ...
        if not raw_query:
            return Response({"detail": "لطفاً عبارت جستجو را وارد کنید."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if not _check_rate_limit(cache_conn, user_identifier):
            return Response({"detail": "تعداد درخواست‌ها بیش از حد است. لطفاً بعداً تلاش کنید."}, status=status.HTTP_429_TOO_MANY_REQUESTS)

        # 2. Caching: تولید کلید کش بر اساس تمام پارامترهای جستجو
        ...
        if cached is not None:
            # اعمال pagination روی نتایج کش شده
            if entity_type:
                ...
                return Response({"query": raw_query, "results": paginated_results}, status=status.HTTP_200_OK)

        results = {}

        # 3. ذخیره تاریخچه جستجو (فقط برای کاربران لاگین شده)
        ...
        def build_search(doc_class, fields, status=None, from_date=None, to_date=None):
            ...
            # فعال‌سازی جستجوی فازی (Fuzziness) برای بهبود نتایج
            ...
            if from_date or to_date:
                ...
                # این فیلد date در AppointmentDocument و CaseDocument وجود دارد.
                q &= Q("range", date=date_range) 
            
            # 💡 NOTE: در اینجا باید منطق دسترسی پیاده‌سازی شود
            # اگرچه این بخش فعلا به دلیل AllowAny کامل نیست، ساختار درست است.
        ...
        # 5. اجرای جستجو روی تمام موجودیت‌ها
        all_searches = {
            ...
            "appointments": lambda: build_search(AppointmentDocument, ["lawyer_name", "status", "date", "session_type"], status=status_filter, from_date=from_date, to_date=to_date),
        ...
        if entity_type in all_searches:
            # جستجو برای یک موجودیت خاص
            items = all_searches[entity_type]()
            page_items, paginator = paginate_list(request, items)
            results[entity_type] = page_items
            response = paginator.get_paginated_response({"query": raw_query, "results": results})
        else:
            # جستجوی عمومی (همه موجودیت‌ها)
            for key, func in all_searches.items():
                items = func()
                # در جستجوی عمومی، pagination فقط روی نتایج برگشتی هر دسته اعمال می‌شود
                page_items, _ = paginate_list(request, items) 
                results[key] = page_items
            response = Response({"query": raw_query, "results": results}, status=status.HTTP_200_OK)

        # 6. ذخیره نتیجه نهایی در کش
...
class SearchHistoryListView(APIView):
    ...
    def get(self, request):
        ...
        cached = _safe_redis_get(cache_conn, cache_key)
        if cached is not None:
            return Response({"history": cached}, status=status.HTTP_200_OK)
...
class SearchSuggestionsView(APIView):
    ...
    def get(self, request):
        ...
        cached = _safe_redis_get(cache_conn, cache_key)
        if cached is not None:
            return Response({"suggestions": cached}, status=status.HTTP_200_OK)

        # 2. Querying (تاریخچه اخیر کاربر + عبارات پرطرفدار عمومی)
        try:
            # پیشنهادات اخیر کاربر
        ...
        # 3. ترکیب، حذف موارد تکراری، و فیلتر
